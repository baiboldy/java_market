...
import kz.mandarin.shopping_center.dto.AddToCartRequest;
...
public class CartService {
	...
	public CartItem addToCart(Long userId, AddToCartRequest addToCartRequest) {
		User user = userRepository.findById(userId).orElseThrow(() -> new RuntimeException("user not found"));
		Product product = productRepository.findByIdAndStatus(addToCartRequest.getProductId(), ProductStatus.AVAILABLE).orElseThrow(() -> new RuntimeException("product not found"));
		if (product.getStockQuantity() < addToCartRequest.getStockQuantity())
		...
		if (existingItem != null) {
			existingItem.setQuantity(existingItem.getQuantity() + addToCartRequest.getStockQuantity());
			return cartItemRepository.save(existingItem);
		} else {
			CartItem newItem = new CartItem();
			newItem.setCart(cart);
			newItem.setProduct(product);
			newItem.setQuantity(addToCartRequest.getStockQuantity());
